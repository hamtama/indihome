...
if ($_POST['rowid']) {
	...
	$sql = "SELECT * FROM tb_kasus
            ...
            INNER JOIN tb_bobot ON tb_kasus.id_bobot = tb_bobot.id_bobot WHERE id_kasus ='$id'";
	$result = $mysqli->query($sql);
	foreach ($result as $baris) {
	?>
	<form action="aksiedit.php" method="post" role="form">
		<div class="form-group field item row" >
			<input type="hidden" name="id" value="<?php echo $baris['id_kasus'] ?>">
			<label for="kode_kasus" class="col-form-label col-md-3 col-sm-3 label-align">Kode Kasus <span class="required">*</span></label>
			<div class="col-md-6 col-sm-6">
				<input type="text" name="kode" value="<?php echo $baris['kode_kasus'] ?>" required="required" class="form-control has-feedback-left">
				<span class="fa fa-qrcode form-control-feedback left"></span>
			</div>
		</div>
        <div class="form-group field item row" >
            <label for="nama_gejala" class="col-form-label col-md-3 col-sm-3 label-align">Nama Kasus <span class="required">*</span></label>
            <div class="col-md-6 col-sm-6">
                <input type="text" name="kasus" value="<?php echo $baris['kasus'] ?>" required="required" class="form-control has-feedback-left">
