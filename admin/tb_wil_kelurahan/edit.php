...
if ($_POST['rowid']) {
    ...
    LEFT JOIN tb_provinsi ON tb_kelurahan.id_provinsi = tb_provinsi.id_provinsi
    LEFT JOIN tb_kabupaten ON tb_kelurahan.id_kabupaten = tb_kabupaten.id_kabupaten 
    LEFT JOIN tb_kecamatan ON tb_kelurahan.id_kecamatan = tb_kecamatan.id_kecamatan
    WHERE tb_kelurahan.id_kelurahan ='$id'";
    $result = $mysqli->query($sql);
    foreach ($result as $baris) {
?>

<form role="form" method="post" action="aksiedit.php">
    <input type="hidden" name="id" value="<?=$baris['id_kelurahan']?>">
	...
	<div class="form-group field item row" >
		...
		<div class="col-md-6 col-sm-6">
			<input type="number" name="kode_kelurahan" required="required" class="form-control has-feedback-left" value="<?=$baris['kode_kelurahan']?>">
			<span class="fa fa-cogs form-control-feedback left" > </span>
		</div>
	</div>
    <div class="form-group field item row" >
        <label for="nama_kecamatan" class="col-form-label col-md-3 col-sm-3 label-align">Nama Kelurahan<span class="required">*</span></label>
        <div class="col-md-6 col-sm-6">
            <input type="text" name="nama_kelurahan" required="required" class="form-control has-feedback-left" value="<?=$baris['nama_kelurahan']?>">
            <span class="fa fa-cogs form-control-feedback left" > </span>
        </div>
    </div>
