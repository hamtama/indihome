	...
	<div class="form-group field item row" >
		...
		<div class="col-md-6 col-sm-6">
			<select class="form-control has-feedback-left" name="id_rule" id="rule">
                ...
                <?php  
                    $sql_rule = mysqli_query($mysqli, "SELECT * FROM tb_rule_user WHERE 1 ORDER BY kode_rule_user ASC") or die (mysqli_error($mysqli));
                    while ($row = mysqli_fetch_array($sql_rule)) {
                        echo '<option value="'.$row['id_rule_user'].'">'.$row['kode_rule_user'].'</option>
                        option>';
                    }
                ?>
            </select>
